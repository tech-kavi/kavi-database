  ...
  const fetchProgress = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/upload-locks`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
          params: {
            "filters[islocked][$eq]": true,
          },
        }
      );

      const activeLock = res.data.data?.[0];
      if (activeLock) {
    ...
    <div className="mx-auto p-2 lg:p-6 sm:px-6 lg:px-8 w-full max-w-[95vw] space-y-8">
      ...
      <section className="bg-white shadow-lg rounded-2xl p-6 border border-gray-200 hover:shadow-xl transition">
        ...
        <button
          ...
          className={`${isUploadDisabled ? "bg-gray-400 cursor-not-allowed cursor-pointer" : "bg-blue-600 hover:bg-blue-700"
        ...
        {status && (
          <p
            className={`mt-3 text-sm ${statusType === 'error' ? 'text-red-600' : 'text-green-600'
              }`}
          >
            {status}
          </p>
