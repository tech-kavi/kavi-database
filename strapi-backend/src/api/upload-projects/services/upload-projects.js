...
//         filters: { linkedin: { $contains: excelLinkedins } },
//         limit: -1,
//       });

//       const allExperts = await strapi.entityService.findMany(
//       'api::expert.expert',
//       {
//         fields: ['id', 'linkedin', 'documentId'],
//         filters: {
...
// Normalize DB linkedins
const expertMap = new Map();

for (const expert of allExperts) {
  const normalized = normalizeLinkedIn(expert.linkedin);

  if (normalized) {
    expertMap.set(normalized, expert);
  }
}

// Find missing linkedins
const missing = excelLinkedins.filter(
  (linkedin) => !expertMap.has(linkedin)
);
